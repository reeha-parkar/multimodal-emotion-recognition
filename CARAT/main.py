...
def get_args(description='Multi-modal Multi-label Emotion Recognition'):
    ...
    parser.add_argument("--local-rank", "--local_rank", default=0, type=int, help="distribted training")
...
def prep_dataloader(args):
    ...
    if args.aligned: # CARAT's original aligned dataset has 'valid' split
        val_dataset = Dataset(
            data_path,
            'valid',
            args
        ...
        # Named valid split 'val' for my custom dataset, hence the change
        val_dataset = Dataset(
            data_path,
            'val',  
            args
